...
@pytest.mark.parametrize("cli_params, expected_out",
        [([], "usage:"),
         ...
         (["-b", "-c", "data/config.yaml"], "Cannot connect to database DBs/nvcl9-full.db: unable to open database file"),
         (["-b", "-c", "data/empty_config.yaml"], "Cannot load config file data/empty_config.yaml, it is empty"),
         (["-b", "-c", "data/missing_config.yaml"], "Config file data/missing_config.yaml is missing a value for 'tsg_meta_file'"),
